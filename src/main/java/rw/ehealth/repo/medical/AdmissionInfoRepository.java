...
public interface AdmissionInfoRepository extends JpaRepository<AdmissionInfo, Long> {
	 ...
	 * @return the long
	 */
	@Query("SELECT count(h.hospitalId) from AdmissionInfo a Join a.hospital h WHERE h.hospitalId=:hospitalId and a.releasedDate is null")
	long countAdmissionInfo(@Param("hospitalId") Long hospitalId);

	@Query("SELECT a from AdmissionInfo a JOIN a.hospital h WHERE h.hospitalId=:hospitalId and a.releasedDate is null")
	List<AdmissionInfo> allAdmissionInfos(@Param("hospitalId") Long hospitalId);

	@Query("SELECT count(p.patientNumber) from AdmissionInfo t JOIN t.admittedPatient p  JOIN t.hospital h  WHERE p.patientNumber=:patientNumber and h.hospitalId = :hospitalId ")
	long countAdmissionBypatient(@Param("patientNumber") String patientNumber,
			@Param("hospitalId") Long hospitalId);

	@Query("SELECT a from AdmissionInfo a Join a.admittedPatient p Join a.hospital h WHERE p.patientNumber=:patientNumber and  h.hospitalId=:hospitalId")
	List<AdmissionInfo> listAdmissionInfosByPatients(@Param("patientNumber") String patientNumber,
			@Param("hospitalId") Long hospitalId);

	@Query("SELECT a from AdmissionInfo a Join a.hospital h JOIN a.departement d WHERE h.hospitalId=:hospitalId and a.releasedDate is null and d.id=:id")
	...
	@Query("SELECT DISTINCT(h) from AdmissionInfo a JOIN a.admittedPatient p JOIN a.hospital h WHERE p.patientNumber=:patientNumber")
	...
	@Query("SELECT new rw.ehealth.report.AdmissionReport(a,p,count(p.patientNumber)) from AdmissionInfo a JOIN a.admittedPatient p   join a.hospital h WHERE p.gender=:gender and h.hospitalId=:hospitalId group BY p.patientNumber")
